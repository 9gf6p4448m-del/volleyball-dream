...
// 07-30（§十 階段五 段 2，t=0.50）治具前提修正：原三顆注入球的滯空是按 t=0 可及
// （1.3m 圓柱）校準的邊緣球——收斂後備援跑到時球已低於可觸窗，三顆全落地＝測不到
// 行為（備援指派與追球的前置斷言全過，死的是治具球）。各加 vy +1.0 給滯空，
// 讓「備援接手」在當段可及下仍測得到；**所有行為斷言一字未動**。登記於 convergence §5.6。
test('§8⑤：二傳真的追不到第二球時，第二追球者接手把球舉起來（不是沒人補球落地）', () => {
  for (const ball of [
    { x: 2.5, y: 2.0, z: 7.5, vx: 0, vy: 4.5, vz: 0 },
    { x: 3.2, y: 2.0, z: 7.8, vx: 0, vy: 5.5, vz: 0 },
    { x: 0, y: 2.0, z: 8.2, vx: 0, vy: 5.5, vz: 0 },
  ]) {
