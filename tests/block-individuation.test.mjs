...
//
// ★★ 2026-08-08 侵蝕來源已完全歸因（二分 26 個 commit，零殘差）★★
// −69(edcfd94) +24(0818007) −8(9789601) −9(42f106c) +50(4010c26) **−184(6687d8a)**
// +5(18af216) +3(2164670) −32(0a6458c) ＝ **−220**，與 1022→802 精確吻合。
// 1022 本身也查證了：`git log -S "1022"` 命中建檔 commit b940d0b，在該 commit 上實跑
// SETS=40 得 1022/16/1.5656%，與註解逐值吻合 ⇒ **它是真實實測值，不是抄來的**。
//   ‧ 主因 `6687d8a`（攔網時序段2：commit 賭注改讀二傳配分傾向）獨佔 −184 波＝84%。
//     **是行為改善不是退步**——它讓兩人更早進入 `seen=true`，壓縮了「靠反應快慢差異
//     製造個別性」的時間窗；作者在 commit 訊息裡就預告「段3 做成 per-blocker 即可自然
//     還原」，下一個 commit `18af216`（兩翼進候選池）果然把相異率從 0.24% 推回 7.6~11.7%。
//     ⇒ **不要為了把樣本數追回去而改行為**，那是把改善當成 bug 修。
//
// ★★★ 但二分過程挖到一件更該擔心的事：這個警報器的攻擊面被意外擴大了 ★★★
// `7a2bf1f`（08-05 難度重校卷題E）把 commit 攔網手的起跳主時鐘從「隊友動作的下降沿」
// 改成「取樣球的 `predictContactPoint`」——而 read 人格讀的**是同一條**。
// 兩人不共用變數，但**共用了輸入**（全域的球體狀態）⇒ 這是達成「同步線」症狀的另一條路徑，
// 本測試原本要防的只有前者。實測該 commit 讓相異率 8.48% → 1.20%（778→753 波）。
// 後果：**任何改攻擊幾何／球體軌跡的改動，現在都會漣漪到這支警報器**。
// 證據不是推論——`7eb0e43`（內切吃二速骰，與攔網完全無關）確實把它從 1.07% 動到 0.77%；
// 那次 commit 自述「−2.1pp 皆在雜訊內」，但**固定種子下這是確定性位移**，
// 只是幅度小到被當時的統計方法歸為不顯著。
// ⇒ **讀這支警報器變紅時，先問「這一批有沒有動到攻擊落點」再問「攔網是不是退化了」**。
//   分不出這兩者，正是假警報的溫床；而假警報的代價是訓練人忽略這個訊號。
// ⇒ 待裁（Sawmah）：要不要重新設計成「只對攔網改動敏感」（例如固定攻擊幾何當控制變因）。
//   現況仍安全：SETS=50 下 bothN 967／門檻 800（餘裕 167 波）、pct 1.14%／門檻 0.5%。
//   ★上面那句「清算之前就只剩 2 波餘裕」是 08-03 的狀態，**現已不成立**，勿再引用★
