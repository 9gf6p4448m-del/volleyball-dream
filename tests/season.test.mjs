...
test('招募跨屆累積（驗收條款）：第二屆再勝同隊 → progress.wins +1', () => {
  ...
  assert.equal(store.advanceSeason(), true);
  settleAll(); // 第二屆再打——同場次 id、results 已重置
  assert.equal(progressOf(store.loadRecruitment(), 'north-tech').wins, 2, '第二屆再勝 north-tech 必須 +1');
  assert.equal(progressOf(store.loadRecruitment(), 'obsidian').wins, 2);
});
