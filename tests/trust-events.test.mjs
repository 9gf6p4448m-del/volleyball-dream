...
test('後排攻擊資格：pipe 未解鎖者不進後排攻擊池（二傳不舉送分球）', async () => {
  const { attackPointsOf } = await import('../src/sim/ai.js');
  const player = createCareerPlayer('小夢'); // pipe:0
  const g = createGame({ seed: 7, teams: careerTeams(player) });
  // 把 A2 換到後排（index 4＝5 號位；前排＝2/3/4 號位即 index 1-3）
  const rot = g.match.rotations.A;
  const idx = rot.indexOf('A2');
  [rot[idx], rot[4]] = [rot[4], rot[idx]];
  const pts = attackPointsOf(g, 'A', 'A1', 'perfect');
  assert.ok(!pts.some((p) => p.pid === 'A2'), '未解鎖 pipe 的後排 A2 不得進池');
  // 解鎖後回池
  player.techniques.pipe = 1;
  const pts2 = attackPointsOf(g, 'A', 'A1', 'perfect');
  assert.ok(pts2.some((p) => p.pid === 'A2' && p.kind === 'pipe'), '解鎖後 A2 應以 pipe 進池');
});
