...
test('後排攻擊真的會執行：整局出現攻擊線後起跳的扣球（淨空約束彈道）', () => {
  // 迴歸：曾經 pipe/D 球只有「分配」沒有「執行」——直線彈道從攻擊線後過不了網，
  // 全部退化安全球；spikeVelocity 加網口淨空約束後應真扣得出去
  const g = createGame({ seed: 101, setTarget: 15 });
  const ai = createAiState();
  let backSpikes = 0;
  while (g.phase !== 'set_over' && g.tick < 400000) {
    for (const e of stepGame(g, aiCollectIntents(g, ai))) {
      if (e.type === 'TOUCH' && e.kind === 'spike') {
        ...
        const lz = (p.teamId === 'A' ? 1 : -1) * g.actors[e.playerId].z;
        if (lz > 3.05) backSpikes += 1;
      }
    }
  }
  assert.equal(g.phase, 'set_over');
  assert.ok(backSpikes >= 1, `整局應有後排扣球實際執行（實得 ${backSpikes}）`);
});
