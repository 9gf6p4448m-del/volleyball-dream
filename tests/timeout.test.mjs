...
test('W7.1 二輪 提早開賽：暫停窗可縮短到走回位緩衝；普通死球窗/rally 擋下', () => {
  const g = createGame({ seed: 7 });
  // 普通死球窗（無暫停）＝不受理
  assert.equal(resumeFromTimeout(g).reason, 'not-timeout');
  assert.equal(applyTimeout(g, { team: 'A' }).ok, true);
  assert.ok(g.serveReadyTick - g.tick >= TUNING.TIMEOUT_DEAD_TICKS);
  const r = resumeFromTimeout(g);
  assert.equal(r.ok, true, r.reason);
  assert.equal(g.serveReadyTick, g.tick + TUNING.TIMEOUT_RESUME_BUFFER);
  assert.ok(g.events.some((e) => e.type === 'TIMEOUT_RESUME'));
  // 已縮短＝再次呼叫不受理（殘餘 ≤ 一般節拍）
  assert.equal(resumeFromTimeout(g).reason, 'not-timeout');
  // rally 中不受理
  const g2 = createGame({ seed: 11, setTarget: 15 });
  const ai = createAiState();
  while (g2.phase !== 'rally' && g2.tick < 100000) stepGame(g2, aiCollectIntents(g2, ai));
  assert.equal(resumeFromTimeout(g2).reason, 'not-dead-ball');
});
