...
test('A5 含 passType 的錄影整卷重播與逐 tick 還原逐位元相同，direct-v4 拒絕', () => {
  ...
  for (const choose of [t => (t < 31 ? 'LEFT' : 'HIGH'), () => 'LEFT']) {
    ...
    assert.equal(serializeDirectState(replayDirectTape({ simulationVersion: 'direct-v5', initial, commands, endTick: s.tick })), serializeDirectState(s));
    assert.throws(() => restoreDirectGame({ ...initial, simulationVersion: 'direct-v4' }));
    assert.throws(() => replayDirectTape({ simulationVersion: 'direct-v4', initial, commands, endTick: 10 }));
  }
});
