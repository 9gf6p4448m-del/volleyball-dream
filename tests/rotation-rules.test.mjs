...
test('換位時序：發球擊出前不換位（前排 OH 停在輪轉位）、擊出後才跑職責位', () => {
  const g = createGame({ seed: 11 }); // A 先發、B 接發
  const ai = createAiState();
  const b2 = 'B2'; // B 隊 OH，開局 2 區（隊伍視角右前＝世界 x=-3）
  // 發球前：只推進到 serveReadyTick 之前，B2 必須還在輪轉基準位
  for (let i = 0; i < 60; i += 1) stepGame(g, aiCollectIntents(g, ai));
  assert.equal(g.phase, 'serve', '前置條件：還沒發球');
  assert.ok(Math.abs(g.actors[b2].x - -3) < 0.2 && Math.abs(g.actors[b2].z - -3) < 0.2,
    `發球前 B2 不得離開輪轉位（實得 ${g.actors[b2].x.toFixed(2)}, ${g.actors[b2].z.toFixed(2)}）`);
  // 發球後：OH 往左翼職責位（B 隊左翼＝世界 x=+3）換位
  let served = false;
  ...
  assert.ok(g.actors[b2].x > 0,
    `發球後 B2 應向左翼換位（x 實得 ${g.actors[b2].x.toFixed(2)}）`);
});
