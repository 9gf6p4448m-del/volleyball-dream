# Phase 4 W4 結案快照 — 賽制＋場館＋數據＋結算＋L 2.0

> 2026-07-27。工單＝`kickoffs/phase4-w4-implementation-prompt.md`（§0 拍板＋附錄 B 與憲法同效力）。
> 基準 `main@5386c27`（W3 結案 479 測綠）→ 結案 **517 測綠（+38，只增不減）**、vite build 綠。
> 驗證：全套 node:test＋治具多臂 n=150＋Playwright 對 preview 實跑
> （選檔頁→創角→生涯→生涯數據頁→選檔卡片回讀；快速比賽 bo3 局間 huddle→續局；
> 冠軍館燈光秀 tour 鏡頭＋sim 凍結），全程 **0 console error/warning**。
> **工單十件全數交付（含 L 2.0 整包——未滑 4.5）**；宿敵三案待 Sawmah 挑（§7）。

## 0. 待 Sawmah 事項——07-27 當日全數裁決 ✅

1. ~~宿敵三案挑一~~ → **B 案「攀天者」莊敬嶺已落檔**（sky-hawk ace：grade 1 同屆＋
   rival 豁免；王勝翔讓 ace 移 slot 4 降 grade 2＝還魂防線收束、「決賽擊敗即可招」
   不動；周羽辰移 reserves；ace 反讀/情蒐數據面即刻生效）。台詞與三幕＝4.5 劇情輪。
2. ~~真機驗收~~ → **兩項驗訖**（Sawmah 07-27）：冠軍館滿場景真機 60 FPS ✅（題9
   硬驗收過）；五局打滿體力體感 ✅（RECOV_SET_BREAK=0.12 定案）。
3. ~~平衡裁定~~ → **先照實記錄不動參數**（Sawmah：「之後覺得要調整我們再調」）：
   多局奪冠 1% 新帶／OPP 多局要球代價／L 改判空間位移——待多屆實玩體感再議。

## 1. 交付總表（工單 §1 十件）

| # | 區塊 | 內容 | 落點 |
|---|------|------|------|
| 1 | 多槽存檔＋局間存檔鏈（題2/Q8 地基） | 3 槽 key 映射（槽1沿用 `vd-save` 零遷移；`-s2/-s3`）＋存檔頭同步維護與自癒（讀卡不解整包）＋選檔頁卡片（角色名/位置/屆/戰績/身高；空槽「新的夢」；刪檔二次確認）＋可切槽 store 代理（careerScreen 零改動）＋賽末返回帶槽號；局間存檔＝整包 sim state JSON 快照落槽位 mid key，**存讀續打＝不中斷逐 tick 等價（rngState 同步，測試背書）**；存檔離開豁免棄賽、完賽/殘檔自動清理；快速比賽不入槽 | `career/saveSlots.js`（新）／`careerStore.js`（slot 參數＋`createSlotStoreProxy`＋mid API）／`careerScreen.js`／`main.js`／`matchCareer.js` |
| 2 | 分級賽制（Q8 全配套） | 多局系列狀態機：`series`（bo3/bo5）＋`set_break` 相位＋`startNextSet`（唯一局間推進路徑）——體力跨局延續＋局間恢復（`RECOV_SET_BREAK` 0.12 初擬）、氣勢跨局歸零、trust 場內動態延續、換人/暫停額度每局重置、輪轉回開場先發序、發球權逐局交替、決勝局 15 分＋8 分換邊（`SIDE_SWITCH` 事件化＋字卡；物理半場不換＝裁量點 §8-1）；賽制推導 `matchFormatOf`（決賽 bo5／準決賽・宿敵場 bo3／其餘 bo1；純推導零存檔遷移）；bo1 零擾動（同種子逐事件等價測試）；局間 huddle 過場（比分回顧＋教練指示決定論選句＋下一局/存檔離開；S 導師句掛點預留） | `sim/game.js`／`sim/stamina.js`／`career/schedule.js`／`app/matchConfig.js`／`app/matchCareer.js`／`ui/setBreakOverlay.js`（新） |
| 3 | 三館制（Q10） | `VENUES` 三館規格單一事實源：常規館不動／關鍵戰館（貼場五層環繞＋桁架＋藍灰膠地）／冠軍館（穹頂骨架＋上下兩層看台＋滿場觀眾＋國際賽配色地板）；`createArena` 參數化重建＋`court.setFloorPalette`；主客場氛圍＝天鷹橫幅＋客隊應援色塊＋`sfx.setCrowdBias` 音場偏對手（冠軍館中立）；冠軍館燈光秀開場（暗場→聚光逐盞亮→巡場 `tour` 鏡頭；sim 凍結、點擊跳過、演出畢才播情蒐帶、續玩不重播）；降規把手＝attendance→上層 count→桁架/穹頂 | `render/arena.js`（重寫）／`render/court.js`／`render/scene.js`／`render/cameraRig.js`／`ui/sfx.js`／`main.js`／`app/matchLoop.js` |
| 4 | box score（Q9） | `career/boxScore.js` 純函式事件流攔截：全員逐人記帳（得分/攻擊/攔網/ACE/接發分檔）＋S 欄（分配/二次球）＋`buildAceBox`＋`buildMentorFeed`（契約形狀）；單場結算頁（局終第一點＝結算頁、第二點＝返回；我方全員＋對手王牌行＋位置差異行）；L 四欄「改判成功率」定形入帳（表現層 tally→`stats.liberoBox.overrides`）；導師接線上線（賽後鏈 `dueMentorLines` 決定論每場至多一句、events 防重播）；對手 ace 記帳餵情蒐（`recordAceBook` 跨屆累積→對陣畫面「上次交手他扣了 N 分」）；生涯累積頁（屆末封存 `archiveSeasonSummary`＋📊 生涯數據 overlay） | `career/boxScore.js`（新）／`ui/boxScorePanel.js`（新）／`app/matchCareer.js`／`app/matchLoop.js`／`careerStore.js`／`careerScreen.js` |
| 5 | 題3 二次球＋題5 OPP 要球 | 二次球：S 前排＋Perfect 檔→「🎯二次球」（`chooseSetDump` 沿 AI setterDump 同 sim 路徑 t=0.3、零特例、不吃 trust；真值字卡＝實際出手才追蹤，出界不冒認被識破）；OPP 要球：後排一傳起球→「⚡跟上！」浮鈕 0.8s（blockTap 語彙不減速）→`call` Intent（VCR 同錄）→sim 登記 `rally.callPid`——D 球授予決定論 hash（`CALL_GRANT` 0.7 初擬非保證）＋甜蜜區微放寬（`CALL_SWEET_WIDEN` 0.06）＋trust 升降 2×（`CALL_TRUST_MUL` 沿乘係數）；表現層喊「我來！」＋S 回頭；**OH 零新機制（關卷）** | `input/setOptions.js`／`input/matchControls.js`／`sim/game.js`／`sim/trust.js`／`ui/callButton.js`（新）／`app/matchLoop.js` |

> ★ **2026-07-31 憲法修正（組合攻擊卷 段 E）**：上表第 5 列「**OH 零新機制（關卷）**」的裁定範圍**收窄為「球內」**——**死球窗的請求機制不在其內**。
> 理由：請求非指令、經 trust 過濾、玩家只能代表自己。三條缺一即回到原禁令。
> 權威條文與完整論證在憲法檔 `docs/phase5-decisions-RESOLVED.md` §九-A；本行只是交叉索引，勿在此處另立說法。
> **OH 球內要球鈕維持不做。**

| 6 | 題1 請調事件 | `transferCandidates`（gate＝目標 open＋打滿 3 場＋每屆一次）＋教練反問（勝率分版）＋志願分版台詞（「我等你這句話很久了」/勸但尊重）；二選一互斥＝`TRANSFER_USED_EV`（接受）→當屆屆間談話不觸發（換屆前捕捉舊屆旗標）；婉拒僅 asked＝入口收起、屆間照常；「去找教練」對話事件入口（非功能按鈕）；生效下一場（`applyPositionChange` 現成鏈） | `career/positionEvents.js`／`careerScreen.js` |
| 7 | 題6 宿敵 | 三案人設存 §7（未落檔未接線）；資料鉤全落：`applySeasonRoster`/`graduatingAces` 宿敵豁免（`ace.rival` 旗標、合成 def 測試背書）＋sky-hawk 隊級 `rival: true`＋關鍵戰館橫幅/應援指向天鷹＋對陣畫面 🔥 宿敵標記行＋ace 反讀旗標鏈（B-4） | `career/opponents.js`／`career/careerState.js`／`career/schedule.js`／`careerScreen.js` |
| 8 | 生涯結算（Q5） | `careerFinale.js`：隊友具名送別（班底手寫 5 組＋generic 名冊解版本）＋主角版畢業儀式（graduationRitual 逐位聚光鏈單人版、戰績誠實分版）＋三屆總結＋下一章佔位；結算流程＝三屆定格→生涯數據→招募全記錄→關鍵球典藏行→謝幕→送別→儀式→「第一章・完」→回選檔頁；關鍵球回放資料底＝決賽勝利最後一球 VCR（snapshot＋Intent 流）落 `career.finalRally`（回放引擎＋宿敵之戰回放位＝4.5）；雙人畢業量能（阿岩＋阿遠＝一次暗場逐位聚光兩段，測試背書）；W3 債務 5＝`ONCE_EVENT_IDS` 分類＋`playedOnce` 跨屆旗標（debut 類生涯一次；hot-hand/轉位談話每屆節拍保留） | `career/careerFinale.js`（新）／`career/events.js`／`careerStore.js`／`careerScreen.js`／`app/matchLoop.js` |
| 9 | 治具與平衡（題7/8） | 治具多局化：`VD_BO`（強制 bo3/5）／`VD_MULTISET`（賽程推導）／`VD_NO_BREAK_RECOV`（裸延續對照）／`VD_CALL`（OPP 要球近似：頻率 0.6＋grant 0.7 同 matchLoop 公式）／`VD_RIVAL`（反讀臂）；體力曲線收集（各局開局帶＋打滿場數）；數據見 §4 | `tools/balance-sim.mjs` |
| 10 | L 2.0 整包（附錄 B，未滑） | B-1 合併配套（封直線・收斜線／封斜線・收直線／攔手讓開・收吊球＝`digBias{choice,block}` 一指令雙驅動；攔網站線 `BLOCK_SCHEME_SHIFT` 0.9／'off'＝退攻擊線；攔網數值零特例；A2 節奏資產不動）；B-2 兩層讀對（🧱封線成功！／📖讀對了——球走你留的線；Perfect 窗語意不變；中路皆不中；讓開不補償）；B-3 封線影子（`blockShadow.js` 佈陣可視化＝封線暗牆影/留線微亮，非落點提示，隨指令生命週期收）；B-4 ace 反讀（宿敵 ace 專屬 `rivalAcePid`；scoutTally 鏡像 n≥3 佔比≥0.5→改打讓開的線＋誠實字卡；混配套＝反制；人設未落檔＝機制沉睡零擾動）；B-6 建議品質吃標記比重（硬＞軟 n≥2 佔比≥0.4＞預設封斜線）；B-5 暗號手勢＝4.5 勿實作（照拍板） | `input/liberoRead.js`／`sim/ai.js`／`render/blockShadow.js`（新）／`app/matchLoop.js` |

## 7. 宿敵三案人設（題6——Fable 深推理產出，待 Sawmah 挑）

> 核心題：**「排球＝高度」這句教義，怎麼長成一個人**。共同約束：與玩家同屆（第 1 屆
> 一年級）、天鷹學園 ace、教義化身（你高＝鏡子／你矮＝牆——創角身高無關成立）、
> 命名沿命名工程字族（天空/飛翔字族；全名唯一已查核、naming.test 綠）。
> 三案給出三種「高度教」的活法：**教義的肉身／教義的皈依者／教義的律法**。
> 選定後落 `opponents.js` sky-hawk ace（grade 1 同屆＋`rival: true` 豁免旗標；
> 王勝翔留 squad、招募語意不動）；三幕事件與台詞＝4.5 劇情輪，本週零台詞。

### 案 A・雲上人——楊擎霄（OH，193cm）

- **活法：教義的肉身。** 生在高度裡，從未懷疑過。國小就比同屆高一顆頭，球權、目光、
  勝利，一切自然到像呼吸——他信「排球＝高度」不是因為誰教他，而是因為他的人生
  從沒出現過反例。他不輕蔑矮個子；更殘忍——**他從沒想過矮個子算對手**。
  無惡意的傲慢，是最刺人的傲慢。
- **性格**：陽光、大方、贏了會真心稱讚你「打得不錯」——然後下一球照樣從你頭頂碾過去。
  快樂的天才，快樂到沒發現自己站的地方別人要用爬的。
- **鏡子/牆**：你高→他真心高興「終於有人跟我站同一片天」，把你當同類照鏡；
  你矮→他困惑多於敵意——你的存在是他世界觀裡沒有的東西。
- **三幕軸素材（4.5 展開）**：不敗的第一屆→第一次輸給「矮的排球」時信仰出現裂縫→
  第三屆決賽：加倍教義或承認高度之外的排球——他的成長弧＝第一次「看見」對手。
- **屬性稿**（天賦系拉滿）：jump 78／power 70／reaction 62／stamina 64／speed 66
  ／control 62／serve 66／block 64，height 1.93。稱號「雲上人」。

### 案 B・攀天者——莊敬嶺（OH，188cm）

- **活法：教義的皈依者（傳教士）。** 不是生在高處，是把自己練上去的。國中發育晚、
  被當隱形人，高一暑假抽高十公分後，把彈跑練到指節變形。他信高度，因為高度救過他
  ——**曾經在牆的另一邊，爬過來之後，把牆砌得更高**。對矮個子最狠的永遠是爬上來的人：
  「我證明過了。爬不上來，是你不夠想。」
- **性格**：練習量病態、話少而重，勝利時不笑——他不是在贏你，是在跟過去的自己拉開距離。
  倖存者偏誤的化身：他把自己的倖存當成了普世真理。
- **鏡子/牆**：你高→嫉妒的鏡子（你天生擁有他用命換來的東西——他會打得格外兇）；
  你矮→他必須踩下去的牆：如果你能贏，他的整場攀爬就失去意義。
- **三幕軸素材（4.5 展開）**：碾壓→被你逼到牆邊的準決賽→第三幕坦白：
  「我也曾在牆的這一邊。」——教義動搖的不是輸球，是看見有人不爬牆也站得住。
- **屬性稿**（苦行系拉滿）：jump 74／power 72／reaction 60／stamina 76／speed 62
  ／control 60／serve 62／block 62，height 1.88。稱號「攀天者」。

### 案 C・量天尺——顏世衡（OH，190cm）

- **活法：教義的律法（祭司）。** 高度對他不是信仰，是度量衡：攔網高度、擊球點、
  滯空時間——每一分都能還原成公分差。他不嘲諷、不咆哮，只陳述數字：
  「你的擊球點 2 米 89，我的 3 米 21。這 32 公分，就是今天的比分。」
  **數字比嘲諷更冷**——因為數字聽起來像事實。
- **性格**：冷靜精確、賽前情蒐做得比教練細，筆記本上記著每個對手的攔網指尖高度。
  他量全世界，唯獨沒量過「數字之外」這個變數。
- **鏡子/牆**：你高→數字相近的對照組，他反而最感興趣（「同樣的高度，你的打點為什麼
  比我低」）；你矮→統計上的異常值——異常值必須被解釋，否則他的律法有漏洞。
- **三幕軸素材（4.5 展開）**：量你→數字開始失準（你的成長曲線／L 的地板／讀不出的
  假動作）→第三幕：放下尺，或造一把新尺。與 Q9 數據文化咬合最深——
  **他也在量你**（情蒐宿敵數據面的鏡像敘事）。
- **屬性稿**（精算系拉高）：jump 70／power 66／reaction 70／stamina 62／speed 64
  ／control 72／serve 70／block 66，height 1.90。稱號「量天尺」。

### 挑案備註

- 三案皆未落 `opponents.js`、未接任何劇情（工單 §8：立人不寫戲；選定落檔＋接線另輪）。
- 資料鉤已先行落地（選案無關、本週完成）：①`applySeasonRoster`／`graduatingAces`
  宿敵豁免（`ace.rival` 旗標，合成 def 測試背書）②sky-hawk 隊級 `rival: true`
  ③關鍵戰館橫幅/應援指向天鷹（RIVAL_TEAM_ID）④對陣畫面宿敵標記行⑤宿敵場 bo3。
- 名字查核：楊擎霄／莊敬嶺／顏世衡與全專案現有全名零撞（含天鷹現有名單）；
  稱號 雲上人／攀天者／量天尺 未被使用；naming.test 全綠。
- 機制咬合預留：ace 反讀（附錄 B-4）＝rival 隊 ace 專屬；Q9 aceBook「上次交手他扣了
  N 分」已上線——選案後宿敵的數據面自動生效。

## 5. 測試（479 → 517，+38）

新增：`save-slots`（7：key 映射/槽間零洩漏/存檔頭同步與自癒/局間存檔 key/代理同形）、
`match-sets`（6：系列推進/局間收束重置語意/決勝局 15+8 分換邊/**局間存檔決定論
（存讀續打逐事件等價＋rngState 同步）**/賽制推導/bo1 零擾動逐事件等價）、
`box-score`（5：歸因/二次球/導師契約/aceBook 累積/屆末封存）、`call-dump`（4：二次球
選項條件/call Intent 流/甜蜜區放寬/信任 2×）、`transfer`（3：gate 三條件/二選一互斥/
決定論分版）、`rival`（3：遞補豁免/畢業播報豁免/隊級旗標＋bo3 聯動）、`finale`（6：
送別/主角儀式分版/三屆總結/once 分類與跨屆旗標/雙人畢業量能/finalRally roundtrip）、
`l2-scheme`（4：配套語意/B-6 建議品質/B-4 反讀統計/blockScheme 站位幾何）。
`libero-play` 建議斷言依附錄 B-1 更新（配套 key）。

## 8. 實作裁量點（如實記錄待裁）

1. **決勝局 8 分換邊＝事件化不真換**：SIDE_SWITCH 事件＋字卡演出；物理半場/鏡頭/
   操作恆我方視角（真換＝全鏡像大工程且體驗混亂；多數排球遊戲同解）。
2. **RECOV_SET_BREAK=0.12 初擬**：題7 調參序第一位；§4 曲線數據供裁。
3. **CALL_GRANT 0.7／CALL_SWEET_WIDEN 0.06／CALL_TRUST_MUL 2×**：題5 三初擬值，
   治具驗＋試玩體感裁。
4. **BLOCK_SCHEME_SHIFT 0.9**：封線站位偏移（站線幾何自然改變攔網涵蓋——攔網數值
   零特例）；體感歸試玩清單。
5. **B-4 讓開線對映**：封線配套（覆蓋封線＋收縮線）→讓開＝吊球；讓開配套→讓開＝
   斜線強攻。附錄未逐字指明，依「兩層覆蓋後的唯一縫」推導，如實記錄。
6. **多局場賽果記局數**（entry.scoreFor/Against＝setsWon；各局明細落 entry.sets）；
   棄賽敗維持 0:25 舊格式（bo 場棄賽極端情境，未特化）。
7. **結算頁「宿敵之戰回放位」＝資料預留非 UI 空位**（finalRally 結構可擴陣列；
   顯示哲學：不給玩家看空欄）。
8. **局間 huddle＝DOM 卡片版**（比分回顧＋教練指示＋按鈕）；W7 圍圈 3D 演出未接
   （sim 凍結下 tick 驅動的圍圈管線不可用）——嫌單薄則 4.5 演出輪升級。

## 10. 債務與依賴

1. 宿敵選定落檔（Sawmah 挑 §7 三案）→ ace 換人＋grade 1＋ace.rival:true；
   王勝翔留 squad（招募語意不動）。落檔後：ace 反讀/情蒐同屆語意自動生效。
2. 三幕劇情/台詞/N2 支線接線＝4.5 劇情輪（工單 §13）。
3. 關鍵球回放引擎接線＋宿敵之戰回放收藏＝4.5（資料底已落：finalRally snapshot+steps）。
4. 演出債整批 4.5 專輪（§11 優先級 2 全集＋暗號手勢 B-5）。
5. 治具跨屆臂不吃身高揭曉（W2 債務 2 未解，沿滾）。
6. 舊隊情結動態事件跨屆重播（W7 D1 的 events 逐屆重置遺留——本週只修 debut 類，
   此項如實記錄未修）。

## 11. 試玩批次清單（滾動版；工單 §14 原樣滾動＋銷帳）

- **銷帳修正（照工單）**：W3 快照 §11「⚠ 上架前 Phase 6 移除面板入口」——面板已於
  07-27 移除，此行銷掉勿再滾動。
- **優先級 1｜W4 新增**：
  - 五局打滿體力體感（決勝局該不該這麼累——題7 調參的輸入；曲線數據見 §4）
  - 局間 huddle 節奏（DOM 卡片版份量夠不夠——嫌單薄＝4.5 升級 3D 圍圈）
  - 二次球出現頻率手感／OPP 要球窗 0.8s 與 2× 乘子體感（CALL_GRANT 0.7/放寬 0.06 初擬）
  - 請調對話語氣（教練反問分版）
  - L 2.0：配套三選項與封線影子可讀性／ace 反讀被讀穿的體感（宿敵落檔後）
  - 選檔頁/生涯數據頁/單場結算頁 手機版面
  - **冠軍館滿場景真機 60 FPS 復測（題9 硬驗收——不得以常規館數字交差；未達標依
    Phase 0 精神逐項降規：attendance→上層看台 count→桁架/穹頂）**
- **優先級 2｜儀式演出規格（4.5 專輪待辦全集）**：W1-W3 既有全部＋冠軍館燈光秀精緻度
  ／結算演出份量／雙人畢業節奏＋**五位置招牌演出（07-27 拍板，各位置身分機制的
  決定性瞬間，純表現層吃現有 cameraRig/慢動作/幾何骨架管線）**：
  - **OH「被騙的人」**：假動作得手＝慢動作帶攔網手（頭隨視線走/撲錯線/從指尖旁打穿）
    ——欺敵的騙局本身入鏡
  - **S「全場等你的一秒」**：分配面板 diegetic 化＝鏡頭掃場、高 trust 隊友揮手喊球、
    點隊友模型本身＝分配；二次球＝假舉手型→壓球→靜半拍→爆
  - **MB「早到的人」**：搶快賭對＝最高點滯空等球、網帶上方俯視鏡頭
    （OH 俯衝/L 貼地/MB 靜止＝三角鏡頭語言）
  - **OPP「三米線起飛」**：要球兌現＝低機位跟拍後排起動助跑全程；得手落地＝
    與 S 專屬擊掌（信任下注回報可視化）
  - **L「暗號手勢」**：附錄 B-5（三種真實手勢＋攔網手偷瞄點頭）
- **優先級 3**：heights 巨人體感/190 攔網複驗（沿 W3）

## 4. 治具數據（n=150／曲線臂 240 場 bo5；175 主錨；AI 代打；原始報告＝docs/audits/w4-balance-report.txt）

### 題7 體力曲線（bo5；局開局帶）

| 臂 | 第1局 | 第2局 | 第3局 | 第4局 | 第5局（決勝） | 打滿 |
|----|------|------|------|------|------|------|
| 局間恢復 0.12 | 1.000 | 0.816 | 0.643 | 0.496 | **0.440** | 61 場 |
| 裸延續（恢復 0） | 1.000 | 0.718 | 0.459 | 0.324 | **0.312** | 63 場 |

**校準目標兩條皆成立**：決勝局開局帶 0.440 明顯低於首局、高於裸延續 0.312——
`RECOV_SET_BREAK=0.12` 初擬值過關，題7 調參序（局間恢復量＞暫停回復量＞死球速率）
第一位即收斂，未動暫停/死球參數。Sawmah 五局打滿體感為終裁輸入。

### 多局制新基準（VD_MULTISET：決賽 bo5／準決・宿敵 bo3；W7 全關）

| 臂 | 決賽帶 | 奪冠 | 單局對照 | A2 場均訊號 |
|----|--------|------|----------|-------------|
| **OH 主錨** | 21% | 1% | 23%／7% | 殺球 4.89（bo 制場多） |
| S | 37% | 1% | 33%／5% | 殺球 0 ✔・二次球（吊）0.44 |
| MB | 31% | 1% | 25%／3% | 攔網得分 1.60 |
| OPP | 25% | 1% | 17%／2% | 殺球 6.09 |
| L | 33% | 1% | 32%／4% | 起球 15.61／助攻 5.91／續命 3.41 |
| 150cm＋L | 33% | 1% | — | **與 175-L 逐值相同**（L 判定不吃身高再實證） |

- **單局主錨復位檢查＝23%／7%、殺球 3.07——與 W3 逐值相同**：本週全部 sim 改動
  （series/call/counter/blockScheme）對現行單局**零擾動**的最終實證。
- 多局＝奪冠全面壓到 1%：bo5 冠軍戰對 72 級天鷹的多局數學（強者更穩、爆冷變難）；
  AI 代打為下緣基準（真人讀攔網/假動作/位置玩法應優於此線）。**允許新帶如實記錄
  ——是否校準待 Sawmah 裁（不強拉舊錨）。**

### 題8 平衡遺留重測

- **OPP 要球回帶（單局）**：17%／2% → **23%／3%**（殺球 3.79→5.83）——題5 要球
  自然把 OPP 拉回 20% 帶之上 ✔（拍板預期成立）。
- **多局 OPP＋要球＝18%／0%（vs 多局無要球 25%）**：多局制下要球反而下滑——
  攻擊參與大增（殺球 8.81）＝體力消耗大增，跨局延續把代價放大到後段。
  非 bug：**「信任雙倍下注」在多局制的真實代價湧現**（要不要球變成體力管理的一部分）；
  如實輸出待裁。
- S 臂多局 37%（單局 33%）：拍板不調——等二次球頻率＋多局體力體感後的新數據再裁。

### L 2.0 空間重測（單局；附錄 B-6 治具聯動）

| 臂 | 決賽帶 | 奪冠 |
|----|--------|------|
| 不指揮 | 32% | 5% |
| suggest（照配套建議） | **43%** | 6% |
| omni（全知改判上限） | 43% | 10% |
| suggest＋宿敵反讀 | 43% | 7% |
| omni＋宿敵反讀 | 43% | 9% |

- **「不指揮 32%＞照建議 23%」倒掛解除**：B-1 配套（兩層覆蓋）＋B-6 建議品質後，
  suggest 43% ＞ 不指揮 32%（+11pp 反轉）——照建議走首次成為正收益 ✔
  （題8 拉大首選手段奏效）。
- **改判空間位移**：決賽帶 suggest=omni（43%）、奪冠 +4pp（6%→10%）——配套體系把
  「照建議」本身抬高後，改判增益從決賽帶（W3 +7pp）轉移到奪冠層。如實輸出待裁。
- **ace 反讀制衡（中路重扣修正版）**：奪冠 suggest 6→7％／omni 10→9%——修正後
  **零增益化達成**（吊球版曾致 29%/38% 自廢武功增益，已修 fa343cd），數值制衡幅度
  小（反讀僅決賽場 ace 本人攻擊觸發＝樣本佔比有限）。「先拉大再制衡」的制衡端
  以**被讀穿的體感**為主要交付；數值端待宿敵落檔＋4.5 劇情輪再裁。

## 12. 驗收清單自查（工單 §12）

1. `npm test` 517 pass／0 fail（≥479 只增不減）；vite build 綠。✅
2. 選檔頁 3 槽卡片（Playwright 實跑：創角後槽 1 顯示名/位置/屆/身高/戰績、槽 2/3
   「新的夢」；槽間零洩漏＝save-slots 測試）；刪檔二次確認（上膛變紅二段）。✅
3. 冠軍戰五局鏈：燈光秀（tour 鏡頭＋sim 凍結實測）→局間 huddle（bo3 實跑：比分回顧
   ＋教練指示＋下一局→第 2 局 B 發球/歸零全對）→局間存檔離開→續玩逐值銜接
   （match-sets 決定論測試：逐事件等價＋rngState 同步）→決勝局 15＋8 分換邊（測試）
   →結算頁（兩段式點擊）。✅（五局全鏈真機體感＝Sawmah 試玩批次）
4. 關鍵戰館打天鷹：橫幅＋應援色塊＋音場偏向（rival 旗標鏈）；ace 記帳→情蒐
   「上次交手他扣了 N 分」（box-score 測試＋對陣畫面接線）。✅
5. 冠軍館滿場景真機 60 FPS＝**Sawmah 真機實測為準**（Playwright 桌機 165 FPS；
   降規把手已備）。⏳試玩批次
6. S 前排 Perfect→🎯二次球（測試：出現條件/同 sim 路徑）；字卡真值（得手/被識破）；
   OPP 後排→⚡跟上！浮鈕 0.8s；trust 2× 有測試背書。✅
7. 打滿 3 場→去找教練入口→請調生效下一場；當屆屆間不再觸發（transfer 測試：
   gate 三條件＋二選一互斥）。✅
8. 三屆→生涯結算全貌（three-seasons 既有測試＋finale 測試：定格/送別/主角儀式/
   佔位）；阿岩＋阿遠雙人畢業節奏（測試背書）。✅（快進實玩＝試玩批次）
9. 宿敵三案存快照未接線（§7）；天鷹同屆豁免測試背書（rival 測試）。✅
10. 治具報告：多局全臂＋體力曲線＋OPP/S 重測＋L 2.0 重測（§4，如實輸出）。✅
11. commit＋`npm run deploy:pages`。✅（8 個 feature/fix commit＋部署）

## 13. 試玩追修（07-27 當日回饋）

- **玩家=MB 被自由人換下的回場權（Sawmah 拍板 B＋二次確認）**：FIVB 真實規則＝
  自由人替換不算換人、任何死球可換回——板凳「回場」鈕對自由人配對開放
  （`applyLiberoRecall` sim 唯一路徑、不吃換人額度）；按第一下＝「確定換下自由人、
  自己守後排？」二次確認（4 秒窗）；回場後 hold 抑制窗＝本輪次自己守、轉前排即清、
  下一位後排 MB 恢復預設換入（不跨局）。代價全由既有系統誠實承擔：接球屬性差＋
  不吃板凳快回。配套 C 案提醒：被換下那刻字卡講清節拍（去哪/何時回/主動權在哪）。
  測試 +2（libero-recall：合法性/hold 窗）＝519 綠。
- **暫停同一分防重（Sawmah 拍板）**：同隊同一分至多喊一次（`timeoutAtPoint` 分 key；
  sim 與 ⏱ 鈕反灰共用 `timeoutUsedThisPoint` 單一事實源）；兩隊各自可喊、下一分恢復、
  跨局歸零。規則面 FIVB 允許同隊接續喊，但實戰近乎不存在＋遊戲裡效果一次性（連喊
  ＝純體驗傷害）——「敢刪不服務體驗的自由度」判準。測試 +2（timeout-once）＝521 綠；
  舊測試同分連喊假設依新規則改為換分後再喊。
- **快速比賽補板凳鈕組＋封線影子可讀性（07-27 試玩回饋）**：⏩×2 加速鈕與回場鈕
  原為生涯限定——快速比賽選 L 試駕的場外輪次無鈕可按，改一律建立（鈕只在坐板凳
  顯示＝其他位置零干擾）；封線影子暗牆影（黑）在夜賽地板讀不出語意→改琥珀金帶
  （與「🧱封線成功！」同色＝封線色彩語言）＋首次下配套教學字卡（每場一次：
  「金帶＝攔網封住的線・青帶＝留給你接的線」）。
- **封線影子可讀性三輪定稿（Sawmah 拍板紅/綠）**：①顯示窗延長——原隨第一觸收
  （僅球飛行 ~0.5s，玩家盯球來不及看地板）→改死球才收（整段 rally 尾可讀）
  ②紅帶＝攔網封住的線（0xff4d4d 0.38）／綠帶＝留給自由球員的線（0x35e07a 0.32）
  ——普世色語高對比；金/青版實測截圖疊暖橘地板人眼無感（渲染管線本身無誤，
  暴力可見化實證幾何全對）③教學字卡同步紅/綠。Playwright 真實濃度截圖驗證可讀。
- **MB/L 字卡減量（Sawmah：太多有點亂、以體驗為主）**：裁「確認類」留「結果類」——
  ①MB「🧱 起跳！」按鈕確認浮字移除（自己按的零資訊）②L 手選配套確認浮字移除
  （紅綠帶佈陣可視化即確認）③L 同波整流：封線成功卡出過（1.5s 窗）＝讀對/讀反
  不再出（擦手情境曾連發兩張——一波至多一張結果卡）。結果真值卡（賭對/早了/
  封到/讀對）＝職業身分核心，全數保留。
- **MB/L 決策面板貼底模式（截圖實證修正）**：面板預設錨 bottom 90px 在手機橫向
  （螢幕高僅 ~390 CSS px）落在畫面正中——正好擋住攔網第一視角要讀的網對面
  （舉球員/攻擊手助跑），「跳出這個很難判斷攔網時機」病根。zonePanel 加 'low'
  貼底模式（鈕 22px 拇指位＋標題 88px）——MB/L 兩個隔網讀對面的面板採用；
  攻擊/發球/S 面板不動（零擾動）。MB 線索縮單字（🏃左 🏃中）防手機折行。
  852×393 視口 Playwright 截圖實證視線區淨空。
- **影子收帶時機定稿（Sawmah：留到死球太長、殘留到下一波）**：我方第一觸
  （這波攻擊的結算點）後再留 1.2s——「讀對了」字卡與帶子同框對照（球真的從
  綠帶進來）→收；新指令下達＝舊排程作廢；死球一律收（出界/攔死保險）。
- **MB「早了」卡補球種原因（七輪）**：時機真值骨架不動（三改拍板）；「早了」分語
  ——對方舉球低弧（e.power<0.5＝sim 快攻同判準）＝「快攻比你更快——再搶半拍」
  （賭局課）／高球＝「早了——是高球，等它下來」（執行課）。快攻＝猜與讀、
  高球＝移動＋自抓時機的雙路線語氣就位。
