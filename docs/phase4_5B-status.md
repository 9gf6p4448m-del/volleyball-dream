...
>
> **✅ 07-28 Sawmah 裁定：4.5（A＋B）正式關卷。** 60 FPS 追蹤（試玩截圖曾見 35FPS，
> 場景未定位）與平衡帶重校準（卡死修復後 23%→29%）移入滾動清單、之後再調；
> 追修窗共消化十一筆（§13）。下一站＝4.6 回放引擎（帶本快照去 Claude.ai 生工單）。
...
- **S diegetic「看不到所有攻擊手」（Sawmah 回報）**：病根＝sset 鏡位錨在玩家座標
  ——S 深位接應時鏡頭跟人跑到後場、看向更深處，攻擊手整排落在鏡頭背後
  （投影 NDC z>1＝熱點被藏＝**選項點不到**，比看不到更嚴重）。修三層：
  ①鏡位改錨**球場幾何**（網前定點 (0, 2.4, side·0.6) 回望半場 (0, 1.0, side·4.6)）
  ②分配窗 FOV +12（55→67，兩翼散開時仍入鏡）③熱點投影出界＝夾回螢幕邊緣
  （36px 內距——任何極端站位下選項恆可點）。Playwright 三個決策窗連續採樣
  ＝四熱點全數在畫面內。**教訓：錨「人」的鏡位遇上自由走位必炸——演出鏡位
  一律錨場景幾何，人只當注視目標。**
- **MB 俯視鏡在不相干得分上播出（Sawmah 回報）**：武裝解除規則原只認「對手觸球」
  ——攔到球彈回我方、重新組織打死的得分也冒領了「早到的人」演出。改**任何後續
  觸球即解除**（成因與得分必須同一拍）＋擦手（graze）不武裝；測試同步更新
  （規格變更非遷就）。
- **「單人攔網感」（Sawmah 回報）**：sim 合攔判定未動——病根＝攔網待命牆姿
  （setHold 播序列 t=0 幀）與起跳共用 block 序列，蹲載入插在 t=0 害整排待命
  攔網手變蹲姿、舉手之牆消失。拆序列：`block`（hold 源）t=0 還原 blockUp 舉手；
  重量感（蹲→蹬→滯空→落地）移到 `blockJump`，僅 BLOCK_TOUCH 實跳觸發。
  **教訓：改共用動作序列前先查 hold 消費端——t=0 幀就是某人的常駐姿勢。**
- **MB 演出範圍擴大（07-28 Sawmah 拍板）**：「早到的人」不限搶快——玩家 MB
  **攔網直接得分皆給**（結實封到＋那一拍直接終結；擦手仍不算）。搶快只是
  達成路徑之一，攔死本身即 MB 的身分時刻。
- **二次球熱點改錨畫面下緣中央（07-28）**：sset 鏡頭架在 S 身位＝自己的投影
  常出鏡、🎯被夾到角落——「自己出手」固定下緣中央（鏡頭這一側＝我自己）。
- **第四道招牌演出「邊線是我的」（07-28 Sawmah 拍板 A 案）**：受控者殺球/吊球
  BALL_IN 且落點咬線 ≤0.25m＝貼地低機位落點特寫（球痕咬白線、從場內側望向線）。
  綁事件（任何攻擊位置）；不覆蓋更特定的已武裝演出；冒領防護/頻率框架一體適用。
  門檻定值＝`tools/line-kill-probe.mjs` 實測（AI 亂打 8.4% 殺球≈場均 0.4 次；
  簡化模式選「區」非描點——玩家可控的是「敢選邊線區」，咬多緊歸散佈；嫌多嫌少
  重跑 probe 改 `SIG_LINE_M`）。同時拍板：「被騙的人」維持綁事件不鎖 OH。
- **發球定點鎖（07-28）**：發球員發球階段搖桿失效＋自動站回發球區（servePosition
  與 sim 佈陣同源；先前可走進場內發球）——輸入層攔截；實測按住方向鍵位移 0。
- **S 介面可辨識性（07-28）**：喊球者＝金圈 76px＋「🙋 這球給我！」徽章（不必找背號）；
  二次球＝下緣中央金框膠囊鈕。
- **AI 站位撞號卡死（07-28，唯一動 sim 的追修）**：`dutyPosition` 槽位按角色寫死
  ——S＋OPP 同前排（六人輪轉必然出現）目標點相同，目標追蹤與 separateTeammates
  推拉平衡＝兩人卡死互抖。改整排指派＋撞號拿最近空槽（輪轉序決定論）；
  無撞號輪轉逐值維持原行為。572 測綠＋治具守門 n=100：決賽帶 26%/奪冠 6%。
- **對向穿越頂牛死鎖（07-28 二筆，動 sim）**：站位交換交叉路徑（OH↔OPP 對穿）
  下，separateTeammates 純徑向推力（0.16/tick ≥ 步速）且零側向分量＝互相堵路
  誰也繞不過（截圖實錘：三攻擊手 0.55m 等距鏈狀卡住）。加**切向滑移**
  （SEP_SLIDE 0.6、垂直連線固定手性＝決定論）：對衝錯身而過；距離 ≥0.55m 的
  靜止同伴零擾動（測試背書：死鎖情境 600 tick 內完成錯身——舊版此測必掛）。
  **平衡帶如實記錄（待 Sawmah 裁）**：兩筆卡死修復合計決賽帶 23%→29%、
  奪冠 7%→5%（n=300）——原基準摻著「球員卡死＝表現折損」，解開後為真實力帶；
  照 W4 慣例不強拉舊錨、是否重校準另裁。
- **懸念入卷（07-28 Sawmah）**：攻擊「瞄準點/描線」機制——簡化模式只選區、
  無精準落點控制；要加就是 gameplay 新機制（動 sim、增操作負擔），如何加而不
  變難＝**Phase 5 kickoff 討論題**，本輪不動。
