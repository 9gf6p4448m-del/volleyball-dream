...
## 9. 代理測試（Sawmah 未試玩，Fable 代驗——2026-07-26 追加）

1. **手機效能代理**：Playwright＋CDP CPU 節流 6×（中階手機等級）實跑儀式全程——
   聚光段 144fps／長高補間段 132fps／定格段 165fps，遠高於 60 底線。真機 GPU 未驗，
   但單人＋單聚光的負載量級風險極低。
2. **160 vs 190 體感代理**（治具新增 A2 場均輸出，n=150）：殺球 2.90 → 3.70/場
   （+28%）＝攻擊端身高差明確可感。**攔網端 AI 代打結構上量不到**：攔網觸球 94%
   集中雙方 MB（Phase 1 攔網分工＝P3 正對球；A2 主攻 20 場僅 12 次觸球）——OH 的
   攔網體感只在真人按攔網鈕時顯現，機制面（blockReach 線性吃身高、game.js:836 直接
   消費）由單元測試背書。順帶觀察：AI 對 AI 之下 ACE 恆 0（發球必被碰到）——
   ace 統計欄位對 AI 代打恆空，真人用飄浮/跳發才可能非零。
3. **文案冷讀**（fresh sonnet，8.5/10）：中國用語/翻譯腔＝無；教練口吻一致；交接與
   轉授「亡靈不開口」處理判定不尷尬、情感重量足。唯一具體弱點＝主攻存疑句跳出
   「先讓我看到你的 X」句型——已採納改為「先讓我看到你的線路，不只一條」。
4. **弱隊身高 -4cm 假設實驗**（北原/白浪/青嵐暫時 -4cm→175 錨 n=150→已還原）：
   決賽帶 23%→27%（偏高出帶）、奪冠 7%→3%（偏低出帶）——**證實 §5「本週不動」
   裁定：要做寫實化修正就得整組重新校準**，不是免費的。
