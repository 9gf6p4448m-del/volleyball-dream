...
> **2026-09-26 使用者裁定（方向改版，direct-v8）**：所有動作的觸球判定改為 Free Ball 式「規則判定」——位置＋按鍵時機決定結果（完美／普通／差），物理只負責畫面；接球自動送往舉球區、有可見的接球範圍；納入魚躍撲球；快球接球可在練習場使用慢動作。本段取代下文「不放大隱形觸球區、不保底救球」、「力量來自助跑、身體與揮擊速度，不按鍵直接指定成功」、「球仍須碰到可見的手臂或手掌…不依選項直接把球傳送到預設落點」（2026-09-24 滑動選扣球段）、「球仍依前臂實際法線反彈，不傳送到指定落點、不保底」（direct-v4 段），以及技術段「碰到後依相對速度/法線反彈，不把球保送至指定落點」。驗收段「早晚/角度/身高/起跳改變結果」仍然有效，但改以規則判定驗證（按鍵早晚、位置、身高與起跳高度改變判定結果），不再以物理反彈驗證。走位、跑速、起跳等身體運動仍由模擬計算，作為規則判定的輸入。以下原文保留作歷史。總計畫與待裁題見 `docs/kickoffs/direct-v8-rules-plan.md`；Free Ball 查證見 `docs/audits/2026-09-26-freeball-research.md`。
