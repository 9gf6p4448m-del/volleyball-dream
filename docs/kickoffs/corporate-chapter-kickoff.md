...
3. **批 3 賽季迴圈**：企業賽季的出戰入口/名次表/戰績顯示（careerScreen 章節分支，
   ...
   瀏覽器實開截圖。★批 2 修補送審追蹤項（併入本批）：①隊名/球衣色的企業章顯示
   缺端到端斷言（careerScreen 905/1852/266 一帶）②careerScreen:2163-2179 的
   「📊 回看三年的數據」文案與停用「🎓 謝幕」鈕未分流——謝幕鈕在企業章年限=1 時
   不可達，但 CHAPTER_SEASONS 一延長就冒出，本批要嘛分流要嘛測試釘住★
   ★批 1 覆審 MEDIUM 記檔：corporations.js 無 `scoutRead` 欄
