...
## 批 2 覆審二輪交辦（2026-08-27 APPROVE 0C/0H/3M/3L 的處置）

- M1（末季無收束路徑）＝已修：收束佔位改吃 store.proCareerOver()（SSOT），滿十年
  結算後直接收束、文案分退休/滿年限兩態；M2（退休確認卡重入）＝已修：retireConfirmOpen
  旗標＋確定鈕無條件關卡；M3（數據頁「封存未推進」窗口同屆算兩次，pre-existing）＝
  已順修：live 列同屆已封存不 push；L3（回填屆數核對守衛）＝已加。
...
批 4（屆間成長通道）blocked on Sawmah 逐案拍板（硬規則 3：招式與代價）——批 5
（謝幕卡）先行，批 4 拍板後補。批 3 覆審另留一題敘事拍板：曾與王勝翔同隊者轉出後，
日後對上蒼羽會收到敵隊變體台詞（「你比我慢了四年」）——對前隊友說不成立，屬品味題；
連同「宿敵年度重逢輕量句」一起等 Sawmah 裁。
...
- **批 2 必辦**：①舊職業存檔一次性回填——contract（隊階底薪＋入章屆數；否則續約
  公式吃 undefined 傳染）**與已結算季的 proFinish**（線上 5e50b15 存檔可能已封存
  「職業元年」，該筆無 proFinish；results 仍在、還救得回，但**推進鈕一上線第一次
  推進就清空 results＝永久遺失**——回填必須與推進鈕同批且先落地，送審員補抓 MEDIUM）②UI 單年文案全清（「職業元年・完」「續約談判・敬請期待」
...
七批全上線（批 1 年迴圈地基 / 批 2 續約迴圈＋回填 / 批 3 轉隊 / 批 4A 宿敵敘事 /
批 4B 屆間三選一 / 批 5 謝幕卡；批 4 拆 4A/4B、批 5 先行＝調序紀錄）。每批動手前
凍結、fresh opus 對抗式覆審、修補反駁式送審（上限 3 輪皆收斂）。
**送達**：main c3a87d0；gh-pages 207d7ea（2026-08-27 02:07）；測試 1991→2057；
sim-hash 34772c06e02243fd 全卷逐值不動。
**試玩入口**：?devpro=<隊id> 直達職業章（八隊 id 見 proTeams.js）；PWA 記得
Ctrl+Shift+R。文案與提案數值（薪水表/係數/信任加成/傳承+2/聲望+6）全屬試玩回饋即改。
