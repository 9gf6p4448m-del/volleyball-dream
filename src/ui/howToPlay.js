...
export const HOW_TO_PLAY = [
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          {
            // matchControls.js:739-747 serveZones 四落點；servePanelItems（08-28 重做＝式切換）
            term: '落點四選一',
            desc: '深左／深中／深右／短球。想發飄浮或跳發＝先點「飄浮」／「跳發」切過去（整排落點跟著換色），再按一次切回普通。',
          },
          {
            // matchLoop.js:1259-1292 追發入口＋第二層（含飄/跳變體）；matchControls.js:867-883
            // 名單＝對方當前輪轉的後排三人；閘 matchConfig.js:268（大學傳授 events.js:72-82）
            term: '🎯 追發（大學章學到）',
            desc: '發球面板多這個入口：點開列出對方後排三個人的名字＋目前體力（喘的人標🥵），點誰就發到誰站的位置——專打累的人、破壞對方陣容。球式沿用你在上一層切的飄浮／跳發。',
          },
        ],
      },
      {
        // matchControls.js:196-212 滯空窗內第二次拖曳＝變向（只認方向、拖太短不算）；
        // game.js:741-746,851,939 力量打折＋體力另收；retargetEligible＝技術已學＋非重度疲勞
        heading: '二段時間差（職業章學到）',
        items: [
          {
            term: '滯空中再拖一次',
            desc: '出手選了區、人還在空中時，往別的落點再拖一次＝空中改打那裡。球會變慢變高、體力燒很兇，重度疲勞時使不出來；改打的線上有人站著照樣被攔。',
          },
        ],
      },
      {
    ...
    sections: [
      {
        ...
        items: [
          ...
          {
            // matchStage.js:154-155 📢浮鈕；matchLoop.js:2936-2952 子面板；ai.js:1073-1078 窗＝
            // 我方一傳剛起、二傳未觸球；game.js:187 AUDIBLE_FAIL_MUL=2（沒得分信任扣加倍）
            term: '📢 改叫（職業章學到）',
            desc: '你不是二傳也能下組合指令：我方一傳剛起、二傳還沒碰球的那一瞬鈕會亮，點開選交叉／時間差／B快，直接生效、二傳不能拒絕。賭的是地位：那一波要是沒得分，信任扣加倍；得分則照常計、不另外加獎。',
          },
        ],
      },
      {
      ...
      },
      {
        ...
        items: [
          ...
          {
            // matchLoop.js:1227-1230 兩選項；matchControls.js:633-639 掛在下一跳（手動/自動皆吃）；
            // game.js:1383-1398 擦頂壓死、:1420 擦側×1.6、:1442 正面×0.7（B7-3 第二次修訂）
            term: '✋ 壓手封直線／封斜線（大學章學到）',
            desc: '讀舉球面板多的兩個紅色選項：手蓋過網去壓球，賭這球會擦到你手頂——賭對直接壓回他們半場，擦到手側或正面反而比直著的手吃虧，而且手在網上有觸網犯規的風險。選了就自動掛在這一波接下來的那次攔網起跳上，自動起跳也算。',
          },
        ],
      },
      {
  ...
  },
  {
    ...
    sections: [
      ...
      {
        // careerScreen.js:1071-1118 對陣卡區塊與提示句；growth.js:74 desc；
        // 反常線折扣＝sim 既有 0.6 倍（scoutBlockMul，批 4a 零新判定）
        heading: '餵線（職業章學到）',
        items: [
          {
            term: '🎯 餵線——對手眼中的你',
