...
// ③ 起步前多久才亮起來（tick）。
// ★ 2026-08-05 已量、維持 90（1.5s）★ 原註記為「暫定值，等 Sawmah 試玩後再定，沒有量過」，
// 稽核 08-03 據此懷疑「顯示條件疊三層 AND，視窗可能窄到玩家看不到」。實測 6 局 174 次出現：
//   每次連續顯示 p25 2.50s／**p50 2.80s**／p75 2.85s，**過短（<0.5s）的比例 0.0%**
//   四種狀態的時間分配：倒數 58.0%（每次 1.51s）／「跑！」28.6%／
//   「現在跑！」8.4%（0.22s）／「起跳時機自己抓」5.0%
// ⇒ 一半以上的時間是**倒數**，正是這個值要買的東西（「夠你看一眼再決定要不要退回去拉開」）。
// 「現在跑！」只有 0.22s 不是問題——它前面已經給了 1.5s 倒數，不是要玩家在那 0.22s 內反應。
