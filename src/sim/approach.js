...
//
// ★ 2026-08-07 Sawmah 改判：玩家沒按時**照樣擲骰**（回到 CROSS_RATE 30%）★
//   舊設計是「玩家側連預設都不擲骰」（理由＝真實排球沒骰子、且混入自動切會害玩家
//   學不到因果）。真人實測推翻了它：不按的球一律直線 ⇒ **OH 的球路比這個功能上線前
//   更單調**，等於用「多一顆鈕」換走了原有的變化。可學習性那條理由仍然成立，但它的
//   解法是「讓玩家看得出對手是 read 還是 commit」（賽前情報層，未做），不是拿掉骰子。
//   ⇒ `matchLoop` 開窗時不再預寫 `cutCall`；只有按下去那一波才帶 `forcedCut`。
