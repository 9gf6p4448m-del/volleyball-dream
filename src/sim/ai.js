...
function arbitrate(game, team, landing, excludeId, formationExempt = false) {
  ...
  for (const pid of rot) {
    ...
    //
    // ★ 為什麼是「拿掉」而不是「調小」★ 上面 `:694` 對 `formationExempt===true`
    // 已經整個 `continue` 掉二傳了 ⇒ 這條 ×3 **本來就只在殺球 dig 這條路徑上生效**，
    // 而它的理由（「他接了就沒人舉球」）在對方重扣的 dig 上本來就不成立——真實排球的
    // 二傳在後排就是要挖球，挖起來再由別人舉。理由不成立的懲罰不該留一個折衷值。
    //
    ...
    //   單波整體贏球率 89.1% → 82.6%。這不是零代價的修正。
    //   生涯難度**實質不動**：配對奪冠率差 −1.0±1.7 / +0.0±2.5 / +2.0±4.3 pp，全在噪音內。
