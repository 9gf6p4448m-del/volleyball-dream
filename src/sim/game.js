...
export const TUNING = {
  ...
  // ★落地過程（留著，因為它是「這兩處沒有多消費 rand」的唯一證據）★
  // 兩個乘數先以中性值 1.0 落地跑過一輪，sim-hash 逐值不變 ⇒ 證明乘數只改結果、
  // 沒動到隨機序列；確認之後才調到下面的幅度、並重立基準 34772c06e02243fd。
  //   幅度由 tools/press-cost-sweep.mjs 掃出（2768 個配對接觸點，2026-08-24）：
  //   動手前寫死的判準＝「全樣本淨值最接近 0 或略負、且 top 淨值仍顯著為正」，
  //   1.6/0.7 是唯一同時滿足的一組——全樣本 −1.1pp（雜訊內）、擦頂 +22.0pp（顯著）、
  //   擦側 −10.1pp（顯著）、正面 −6.8pp（顯著）。
  //   ★對照：中性值 1.0/1.0 的全樣本是 +4.7pp 顯著正＝白拿，那就是要修掉的東西。★
  // ★★ 2026-08-31 壓手語意重定（Sawmah 討論定案，推翻 08-24「正面當成本」）★★
  // 真實排球的壓手：手面朝下 ⇒ 手頂與**正面**都是優勢（球被折進對方場內）；
  // 專屬代價＝①觸網 ②「吞下去」——手臂拱過網，拱形下緣與網帶之間的低窗，
  // 貼網低平球從手與網的縫隙鑽進去（台灣排球俗名；國際上預防端叫 sealing the net）。
  // 「擦側=打手」稅退役：打手出界瞄的是任何牆的外緣，不是壓手專屬（08-24 那是
  // 為了記帳需要發明的成本項）。平衡判準同步修訂：接觸後 EV 允許小幅為正
  // （+3~6pp）——壓手綁封線承諾，接觸前已付過「賭錯線整手落空」的稅，
  ...
  // ★ 職業章批 4c「二段時間差」（acceptance-pro-batch4c.md F2；2026-08-26 Sawmah
  // 拍板丙＝純幾何降級）★
  // 扣球起跳後、滯空窗內的第二段變向（intent.retargetAim；只有玩家輸入層會帶，
  // AI 無觸發路徑）。本質是**變向選擇**（打哪裡），不是時機判定——不讀
  // 「什麼時候變向」（B7-3 教訓：時機軸整條是負的，不做按早按晚）。
  // 拍板丙：機率騙牆層（lane 申報＋retargetP 擲骰）已整層刪除——opus 送審實測
  // 因果不成立（lane 申報實為打點預測、判準退化為攻方幾何開關、全牆同判 96.3%）
  // 且整招負期望（代價 −8.9pp > 騙牆收益 +4.5%）。變向的收益回歸**純幾何**：
  // 球飛新線，牆的真實站位沒蓋到就攔不到——tryBlock 的 bandContact 本來就用
  // 真實球路結算，這層天然存在、零特殊判定。
  //   POWER_MUL＝空中二次發力不完整，力量折損（F2-2 代價之一，平打不分角度）。
  //     折損**折在飛行時間**：T×(1/POWER_MUL)＝球整體變慢、弧變高、落點不變
  //    （spikeVelocity timeMul）。折在 speed 的舊做法被過網淨空下限吞掉
  //    （61% 落點逐值零效果）；折在 T 則任何落點有效。
  //     拍板丙代價降輕：timeMul 1.25→1.1（提案值，未經真人試玩校準）——
  //     純幾何收益弱於已刪的機率層，代價同步降
  DBL_SPIKE_POWER_MUL: 1 / 1.1,
...
function executeTouch(state, intent, player, actor, ev, dist = 0) {
  ...
  // 批 4c（拍板丙）：二段變向與視線欺敵**同層擇一、不疊乘**——變向的欺敵本體
  // 就是球飛新線（純幾何：牆的真實站位沒蓋到就攔不到），再疊 gaze 等於同一個
  // 欺敵算兩次；變向時 gaze 讀取整段停用（deceiveP 歸零；同 tool 擇一的先例）
...
// ★ 批 4c 二段時間差（純函式；拍板丙＝純幾何後只管「算不算有變向」）★
// 回傳 true＝變向成立（executeTouch 換 aimPoint、收代價、發 TOUCH retarget 旗標）、
// null＝這一下不算變向（球照第一段 aim 飛、零收費）。**輸入裡沒有任何時間量**
//（F2-1 禁時機軸，B7-3 教訓）。騙牆機率（deceiveP）計算已隨機率騙牆層整層刪除
//——變向的收益現在是純幾何：球飛新線，牆的真實站位沒蓋到就攔不到（tryBlock
// bandContact 用真實球路結算，這層天然存在、零特殊判定）。
// 兩層退化護欄同層並列（sim 端最底層保留）：
//   ① 新落點與擊球點重合（無方向語意的退化輸入）
//   ② 覆審修 3（滑鼠誤觸）：newAim 與 firstAim 逐值相同或 ε 內——第二下沒改任何
//      東西還收全套代價（力量折損＋大耗體力）＝誤觸稅；ε 取 1e-6m（浮點抖動級，
//      無遊戲性意義）。輸入層另有 0.5m 最小位移閘（matchControls，MEDIUM-6），
//      擋的是「有位移但無遊戲性意義」的桌面點按／極短拖曳；本護欄是最後一道
...
function tryBlock(state, toTeam, ev) {
  ...
  for (const p of Object.values(state.players)) {
    ...
    // ★「吞下去」（08-31 Sawmah 提案定案）★ 壓手的手臂拱過網，拱形下緣與網帶之間
