...
export const STAMINA = {
  ...
  // 40-50%（「該換了」的可讀瞬間會出現）、後排/舉球 70-80%
  COST_SPIKE: 0.02,        // 扣球（起跳揮擊）
  COST_JUMP_BLOCK: 0.014,  // 跳攔（每次新窗＝一跳）
  COST_JUMP_SERVE: 0.016,  // 跳躍發球
  COST_DIVE: 0.018,        // 魚躍（撲空也扣——出手即倒地）
  COST_BUMP: 0.002,        // 站立墊球（近零）
  COST_SET: 0.0015,        // 舉球（近零）
  COST_SERVE_STAND: 0.001, // 站姿發球極低（飄浮同）
  COST_RALLY_TOUCH: 0.0012, // 長 rally 每拍：場上全員小額（跑動/預備的持續張力）
  // A3 恢復（死球窗一次性；rally 中結構上無恢復點）
  RECOV_DEAD: 0.004,   // 場上球員死球間隙小回
