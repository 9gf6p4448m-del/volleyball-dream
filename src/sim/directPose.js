...
export function getDirectPose(state, fraction = 0) {
  ...
  for (const [side, sign] of [
    ...
    const hip = point(sign * 0.085, 0.49, 0),
      knee = point(sign * 0.09, 0.27 + crouch * 0.5, 0.025),
      foot = point(sign * 0.09, 0.065 + crouch, 0.04);
    add(`${side}-thigh`, "leg", hip, knee, 0.062);
    add(`${side}-shin`, "leg", knee, foot, 0.045);
    const shoulder = point(sign * 0.145, 0.77, 0);
    let elbow = point(sign * 0.18, 0.6, 0.02),
      hand = point(sign * 0.18, 0.43, 0.04);
    const blend = (a, b, q) => ({
      x: a.x + (b.x - a.x) * q,
      y: a.y + (b.y - a.y) * q,
      z: a.z + (b.z - a.z) * q,
    ...
    if (p.action === "receive" || p.action === "dive") {
      ...
      hand = blend(
        hand,
        point(sign * 0.11, 1.02 + phase * 0.16, 0.15),
        raise * recover,
      );
    }
