...
export const EVENT_DEFS = [
  ...
  // ★文案不說謊（練習賽卷 08-12 事故的教訓）★：對話裡的操作（再拖一次）、代價
  // （力量折損／體力大耗／重度疲勞不可用）、極限（晚起跳的攔網不吃這套）逐句
  // 對得上 sim 實作（game.js executeTouch／tryBlock），不承諾機制沒有的事。
  {
    ...
    lines: [
      { speaker: '總教練', text: '攔網手最怕的不是你力量大——是他跳起來之後，才發現球不在他押的那條線上。' },
      { speaker: '總教練', text: '起跳照舊、揮臂做半套，讓牆照你的助跑方向跳；人還在空中的那一瞬，再拖一次、改打別條線。這叫二段時間差。' },
      { speaker: '總教練', text: '先把醜話講完：空中二次發力，力量回不到全值，體力燒得比誰都兇——腿一沉到重度疲勞，這招就使不出來，對面看你的體力條也知道。' },
      { speaker: '總教練', text: '還有，它不是魔法。等你出手才起跳的牆，看的是球不是你，騙不動。挑那個提早押注的人騙——你決定對手看到什麼，包括你的第一段。' },
    ],
  },
