...
export const TECH_DEFS = [
  ...
  // 職業章批 4c（二段時間差，2026-08-26）：玩家側決策空間第三招，本卷唯一動 sim
  // 扣球判定的地基級批次。扣球起跳後、滯空窗內**再拖一次**＝空中變向（改打點/改線），
  // 騙依第一段身體動向起跳的攔網（sim：intent.retargetAim → game.js executeTouch）。
  // ★本質是「變向選擇」（打哪裡），不是時機判定（按多准）★——沿 B7-3 教訓
  //（時機軸經 08-03 實測整條是負的，acceptance-pro-batch4c.md F2-1 凍結禁做）：
  // 窗內何時變向、第二段按多久，對結果**零影響**；決定成敗的只有你改打哪條線
  //（變向角越大越能騙、封頂 <1）與對面誰已經跳了（晚跳的牆讀真實球路＝免疫）。
  // 代價非純增益（F2-2）：力量折損（DBL_SPIKE_POWER_MUL）＋體力大耗
  //（STAMINA.COST_SPIKE_TWIST），重度疲勞檔（tier 2）使不出來——體力條對手也讀得到。
  {
    key: 'doubleSpike',
    name: '二段時間差',
    desc: '起跳後滯空中再拖一次＝空中變向，騙已起跳的攔網——力量打折、體力大耗，重度疲勞時使不出來',
  },
];
