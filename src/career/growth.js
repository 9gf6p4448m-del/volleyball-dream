...
export const TECH_DEFS = [
  ...
  //
  // ★ 壓手攔網的代價＝**賭接觸區**，不在押方向、也不在時機（B7-3 第二次修訂，08-24）★
  //   sim 端兩顆鈕（tryBlock 非擦頂分支）：BLOCK_PRESS_SIDE_MUL 1.6（擦側橫速更大）／
  //   BLOCK_PRESS_BODY_MUL 0.7（正面攔死打折）；擦頂才壓死。「押方向」只是介面規則
  //   （press 必綁封直線/封斜線，applyMbChoice 對兩者寫一樣的 blockCall）＝對 sim
  //   無邊際代價——舊敘事「放棄 blockPlanTargetX 讀球」已被第二次修訂推翻。
  ...
  { key: 'pressBlock', name: '壓手攔網', desc: '攔網時手蓋過網面，把擦到手頂的球壓死回對方場內——賭的是接觸點：擦側或正面碰上反而更吃虧；出手時得連封線方向一起押' },
  ...
  // 職業章批 4c（二段時間差，2026-08-26；同日 Sawmah 拍板丙＝純幾何降級）：
  // 玩家側決策空間第三招。扣球起跳後、滯空窗內**再拖一次**＝空中變向（改打點/改線），
  // sim：intent.retargetAim → game.js executeTouch 換新落點。
  // ★收益是純幾何的（所見即所得）★：球飛新線，牆的**真實站位**沒蓋到那條線
  // 就攔不到；蓋到照樣被攔——tryBlock 的 bandContact 用真實球路結算，無任何
  // 機率騙牆判定（機率騙牆層經 opus 送審實測否決後整層刪除）。
  // ★本質是「變向選擇」（打哪裡），不是時機判定（按多准）★——沿 B7-3 教訓
  //（時機軸經 08-03 實測整條是負的，acceptance-pro-batch4c.md F2-1 凍結禁做）：
  // 窗內何時變向、第二段按多久，對結果**零影響**；決定成敗的只有你改打的那條線
  // 上有沒有牆站著。
  // 代價非純增益（F2-2）：力量折損（DBL_SPIKE_POWER_MUL，折在飛行時間＝球慢弧高）
  // ＋體力大耗（STAMINA.COST_SPIKE_TWIST），重度疲勞檔（tier 2）使不出來——
  // 體力條對手同樣讀得到。
  {
    key: 'doubleSpike',
    name: '二段時間差',
    desc: '起跳後滯空中再拖一次＝空中變向，把球改打到牆真實站位沒蓋到的空檔——力量打折、體力大耗，重度疲勞時使不出來',
  },
];
