...
// 批2（2026-08-28）起 export：連線層的指令 patch 白名單與錄影帶共用同一份
// （單一事實來源——新增指令槽時只改這裡，錄影與連線同時獲得它）
export const PLAYER_AI_FIELDS = [
  ...
  'cutCall',
  // 2026-08-07：夾塞指令槽（OPP）與 cutCall 同性質——玩家寫、重演算不出來。
  // 與上一行同時登記，不重蹈「上線時漏登記、重播吃掉玩家輸入」那一格。
  'tandemCall',
  // 2026-08-09：B 快指令槽（MB）與上面兩個同性質——玩家寫、重演算不出來。
  // 上線同時登記，不重蹈「內切／夾塞上線時漏登記、重播吃掉玩家輸入」那一格。
  'bquickCall',
];
